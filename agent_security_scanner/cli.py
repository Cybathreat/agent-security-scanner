...
def generate_reports(results: List[ScanResult], args: argparse.Namespace) -> None:
    ...
    if not is_valid:
        logger.error(f"Invalid output directory: {error_msg}")
...
def generate_config(output_path: str) -> None:
    ...
    if not is_valid:
        logger.error(f"Invalid config output path: {error_msg}")
        raise ValueError(f"Invalid config output path: {error_msg}")
