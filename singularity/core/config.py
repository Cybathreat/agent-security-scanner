"""
Configuration loading module for Singularity.
...
class Config:
    """
    Main configuration class for Singularity.
    ...
    def _apply_env_overrides(config: Config) -> Config:
        ...
        Environment variables follow pattern: SINGULARITY_<SECTION>_<KEY>
        Example: SINGULARITY_SCANNER_TIMEOUT=60
        ...
        """
        # Scanner overrides
        if os.getenv("SINGULARITY_SCANNER_TIMEOUT"):
            config.scanner.timeout = int(os.getenv("SINGULARITY_SCANNER_TIMEOUT") or "30")

        if os.getenv("SINGULARITY_SCANNER_MAX_RETRIES"):
            config.scanner.max_retries = int(os.getenv("SINGULARITY_SCANNER_MAX_RETRIES") or "3")

        if os.getenv("SINGULARITY_SCANNER_RATE_LIMIT"):
            config.scanner.rate_limit = float(os.getenv("SINGULARITY_SCANNER_RATE_LIMIT") or "10.0")

        if os.getenv("SINGULARITY_SCANNER_VERIFY_SSL"):
        ...
        if os.getenv("SINGULARITY_LOG_LEVEL"):
            config.logging.level = os.getenv("SINGULARITY_LOG_LEVEL") or "INFO"

        # Output overrides
        if os.getenv("SINGULARITY_OUTPUT_FORMAT"):
            config.output.format = os.getenv("SINGULARITY_OUTPUT_FORMAT") or "json"

        if os.getenv("SINGULARITY_OUTPUT_DIR"):
            config.output.output_dir = os.getenv("SINGULARITY_OUTPUT_DIR") or "output"

        if os.getenv("SINGULARITY_VERBOSE"):
            val = os.getenv("SINGULARITY_VERBOSE")
            if val is not None:
                config.output.verbose = val.lower() == "true"

        # Quality gate overrides
        if os.getenv("SINGULARITY_QUALITY_GATE_FAIL_ON_SEVERITY"):
            config.quality_gate.fail_on_severity = (
                os.getenv("SINGULARITY_QUALITY_GATE_FAIL_ON_SEVERITY") or "critical"
            )
        if os.getenv("SINGULARITY_QUALITY_GATE_MAX_FINDINGS"):
            config.quality_gate.max_findings = (
                int(os.getenv("SINGULARITY_QUALITY_GATE_MAX_FINDINGS") or "0") or None
            )
        if os.getenv("SINGULARITY_QUALITY_GATE_MAX_RISK_SCORE"):
            config.quality_gate.max_risk_score = (
                int(os.getenv("SINGULARITY_QUALITY_GATE_MAX_RISK_SCORE") or "0") or None
            )

        # TAP scanner overrides
        if os.getenv("SINGULARITY_TAP_ATTACKER_LLM_ENDPOINT"):
            config.modules.tap_scanner.attacker_llm_endpoint = os.getenv(
                "SINGULARITY_TAP_ATTACKER_LLM_ENDPOINT"
            )
        if os.getenv("SINGULARITY_TAP_ATTACKER_LLM_MODEL"):
            config.modules.tap_scanner.attacker_llm_model = os.getenv("SINGULARITY_TAP_ATTACKER_LLM_MODEL")
        if os.getenv("SINGULARITY_TAP_ATTACKER_LLM_API_KEY"):
            config.modules.tap_scanner.attacker_llm_api_key = os.getenv(
                "SINGULARITY_TAP_ATTACKER_LLM_API_KEY"
            )
        if os.getenv("SINGULARITY_TAP_JUDGE_LLM_ENDPOINT"):
            config.modules.tap_scanner.judge_llm_endpoint = os.getenv("SINGULARITY_TAP_JUDGE_LLM_ENDPOINT")
        if os.getenv("SINGULARITY_TAP_JUDGE_LLM_MODEL"):
            config.modules.tap_scanner.judge_llm_model = os.getenv("SINGULARITY_TAP_JUDGE_LLM_MODEL")
        if os.getenv("SINGULARITY_TAP_JUDGE_LLM_API_KEY"):
            config.modules.tap_scanner.judge_llm_api_key = os.getenv("SINGULARITY_TAP_JUDGE_LLM_API_KEY")

        # Payload splitting scanner overrides
        if os.getenv("SINGULARITY_PAYLOAD_SPLITTING_ENABLED"):
            val = os.getenv("SINGULARITY_PAYLOAD_SPLITTING_ENABLED")
            if val is not None:
                config.modules.payload_splitting_scanner.enabled = val.lower() == "true"
        if os.getenv("SINGULARITY_PAYLOAD_SPLITTING_COMPLIANCE_THRESHOLD"):
            config.modules.payload_splitting_scanner.compliance_threshold = float(
                os.getenv("SINGULARITY_PAYLOAD_SPLITTING_COMPLIANCE_THRESHOLD") or "0.6"
            )

        # Guardrail fingerprinting scanner overrides
        if os.getenv("SINGULARITY_GUARDRAIL_FINGERPRINTING_ENABLED"):
            val = os.getenv("SINGULARITY_GUARDRAIL_FINGERPRINTING_ENABLED")
            if val is not None:
                config.modules.guardrail_fingerprinting_scanner.enabled = val.lower() == "true"
        if os.getenv("SINGULARITY_GUARDRAIL_FINGERPRINTING_COMPLIANCE_THRESHOLD"):
            config.modules.guardrail_fingerprinting_scanner.compliance_threshold = float(
                os.getenv("SINGULARITY_GUARDRAIL_FINGERPRINTING_COMPLIANCE_THRESHOLD") or "0.6"
            )

        # Adaptive generator scanner overrides
        if os.getenv("SINGULARITY_ADAPTIVE_GENERATOR_ENABLED"):
            val = os.getenv("SINGULARITY_ADAPTIVE_GENERATOR_ENABLED")
            if val is not None:
                config.modules.adaptive_generator_scanner.enabled = val.lower() == "true"
        if os.getenv("SINGULARITY_ADAPTIVE_GENERATOR_MAX_ITERATIONS"):
            config.modules.adaptive_generator_scanner.max_iterations = int(
                os.getenv("SINGULARITY_ADAPTIVE_GENERATOR_MAX_ITERATIONS") or "5"
            )
        if os.getenv("SINGULARITY_ADAPTIVE_GENERATOR_ATTACKER_LLM_ENDPOINT"):
            config.modules.adaptive_generator_scanner.attacker_llm_endpoint = os.getenv(
                "SINGULARITY_ADAPTIVE_GENERATOR_ATTACKER_LLM_ENDPOINT"
            )
        if os.getenv("SINGULARITY_ADAPTIVE_GENERATOR_ATTACKER_LLM_MODEL"):
            config.modules.adaptive_generator_scanner.attacker_llm_model = os.getenv(
                "SINGULARITY_ADAPTIVE_GENERATOR_ATTACKER_LLM_MODEL"
            )
        if os.getenv("SINGULARITY_ADAPTIVE_GENERATOR_ATTACKER_LLM_API_KEY"):
            config.modules.adaptive_generator_scanner.attacker_llm_api_key = os.getenv(
                "SINGULARITY_ADAPTIVE_GENERATOR_ATTACKER_LLM_API_KEY"
            )

        # Virtualization scanner overrides
        if os.getenv("SINGULARITY_VIRTUALIZATION_ENABLED"):
            val = os.getenv("SINGULARITY_VIRTUALIZATION_ENABLED")
            if val is not None:
                config.modules.virtualization_scanner.enabled = val.lower() == "true"
        if os.getenv("SINGULARITY_VIRTUALIZATION_COMPLIANCE_THRESHOLD"):
            config.modules.virtualization_scanner.compliance_threshold = float(
                os.getenv("SINGULARITY_VIRTUALIZATION_COMPLIANCE_THRESHOLD") or "0.6"
            )

        # Encoding bypass scanner overrides
        if os.getenv("SINGULARITY_ENCODING_BYPSINGULARITY_ENABLED"):
            val = os.getenv("SINGULARITY_ENCODING_BYPSINGULARITY_ENABLED")
            if val is not None:
                config.modules.encoding_bypass_scanner.enabled = val.lower() == "true"
        if os.getenv("SINGULARITY_ENCODING_BYPSINGULARITY_COMPLIANCE_THRESHOLD"):
            config.modules.encoding_bypass_scanner.compliance_threshold = float(
                os.getenv("SINGULARITY_ENCODING_BYPSINGULARITY_COMPLIANCE_THRESHOLD") or "0.6"
            )

        # Multilingual scanner overrides
        if os.getenv("SINGULARITY_MULTILINGUAL_ENABLED"):
            val = os.getenv("SINGULARITY_MULTILINGUAL_ENABLED")
            if val is not None:
                config.modules.multilingual_scanner.enabled = val.lower() == "true"
        if os.getenv("SINGULARITY_MULTILINGUAL_COMPLIANCE_THRESHOLD"):
            config.modules.multilingual_scanner.compliance_threshold = float(
                os.getenv("SINGULARITY_MULTILINGUAL_COMPLIANCE_THRESHOLD") or "0.6"
            )

        # Token smuggling scanner overrides
        if os.getenv("SINGULARITY_TOKEN_SMUGGLING_ENABLED"):
            val = os.getenv("SINGULARITY_TOKEN_SMUGGLING_ENABLED")
            if val is not None:
                config.modules.token_smuggling_scanner.enabled = val.lower() == "true"
        if os.getenv("SINGULARITY_TOKEN_SMUGGLING_COMPLIANCE_THRESHOLD"):
            config.modules.token_smuggling_scanner.compliance_threshold = float(
                os.getenv("SINGULARITY_TOKEN_SMUGGLING_COMPLIANCE_THRESHOLD") or "0.6"
            )

        # Grammar-constrained scanner overrides
        if os.getenv("SINGULARITY_GRAMMAR_CONSTRAINED_ENABLED"):
            val = os.getenv("SINGULARITY_GRAMMAR_CONSTRAINED_ENABLED")
            if val is not None:
                config.modules.grammar_constrained_scanner.enabled = val.lower() == "true"
        if os.getenv("SINGULARITY_GRAMMAR_CONSTRAINED_COMPLIANCE_THRESHOLD"):
