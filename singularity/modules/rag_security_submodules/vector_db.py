...
class VectorDBScanner(BaseModule[VectorDBScannerConfig]):
    ...
    AUTH_BYPSINGULARITY_PAYLOADS: List[Dict[str, str]] = [
        {
    ...
    ENCRYPTION_BYPSINGULARITY_PAYLOADS: List[Dict[str, str]] = [
        {
    ...
    async def _test_auth(
        ...
        for payload in self.AUTH_BYPSINGULARITY_PAYLOADS:
    ...
    async def _test_encryption(
        ...
        for payload in self.ENCRYPTION_BYPSINGULARITY_PAYLOADS:
            response = await self._send_message(session, target, payload["payload"])
            if response is None:
                continue
    ...
    def scan(self, target: str, **kwargs: Any) -> ScanResult:
        ...
        result = ScanResult(
            ...
            metadata={
                "mode": "vector_db",
                "auth_bypass_payloads": len(self.AUTH_BYPSINGULARITY_PAYLOADS),
                "encryption_bypass_payloads": len(self.ENCRYPTION_BYPSINGULARITY_PAYLOADS),
