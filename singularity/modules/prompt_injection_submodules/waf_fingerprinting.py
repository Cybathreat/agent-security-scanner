...
class WAFFingerprintingScanner(BaseModule[WAFFingerprintingScannerConfig]):
    ...
    BYPSINGULARITY_TESTING_PAYLOADS: List[Dict[str, str]] = [
        {
    ...
    async def _test_bypass_testing(
        ...
        for payload in self.BYPSINGULARITY_TESTING_PAYLOADS:
            response = await self._send_message(session, target, payload["payload"])
            if response is None:
                continue
    ...
    def scan(self, target: str, **kwargs: Any) -> ScanResult:
        ...
        result = ScanResult(
            ...
            metadata={
                ...
                "bypass_testing_payloads": len(self.BYPSINGULARITY_TESTING_PAYLOADS),
