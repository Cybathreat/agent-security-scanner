...
class CanaryTokensScanner(BaseModule[CanaryTokensScannerConfig]):
    ...
    TOKEN_BYPSINGULARITY_PAYLOADS: List[Dict[str, str]] = [
        {
            "name": "canary_evasion",
    ...
    async def _test_token_bypass(
        ...
        for payload in self.TOKEN_BYPSINGULARITY_PAYLOADS:
            response = await self._send_message(session, target, payload["payload"])
            if response is None:
                continue
    ...
    def scan(self, target: str, **kwargs: Any) -> ScanResult:
        ...
        result = ScanResult(
            ...
            metadata={
                ...
                "token_bypass_payloads": len(self.TOKEN_BYPSINGULARITY_PAYLOADS),
            },
        )
